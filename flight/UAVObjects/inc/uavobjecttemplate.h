...
static inline int32_t $(NAME)ConnectQueue(struct pios_queue *queue)
{
...
{
	return UAVObjConnectCallback($(NAME)Handle(), cb, EV_MASK_ALL_UPDATES);
}
...
static inline uint16_t $(NAME)GetNumInstances()
{
	return UAVObjGetNumInstances($(NAME)Handle());
}

static inline uint32_t $(NAME)GetNumBytes()
{
	return UAVObjGetNumBytes($(NAME)Handle());
}
